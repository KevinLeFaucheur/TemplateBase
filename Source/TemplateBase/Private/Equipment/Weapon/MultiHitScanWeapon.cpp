...
void AMultiHitScanWeapon::Activate(const FVector& HitTarget)
{
	...
	if(MuzzleFlashSocket && InstigatorController)
	{
		...
		bShouldStack ? ActivateHitStack(HitTarget, Start, OwnerPawn) : ActivatePerHit(HitTarget, Start, OwnerPawn);
	}
}

void AMultiHitScanWeapon::ActivateHitStack(const FVector& HitTarget, FVector Start, AActor* OwnerPawn)
{
...
void AMultiHitScanWeapon::ActivatePerHit(const FVector& HitTarget, FVector Start, AActor* OwnerPawn)
{
	if(const UAbilitySystemComponent* SourceASC = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(GetOwner()))
	{
		const FGameplayEffectSpecHandle DamageSpecHandle = SourceASC->MakeOutgoingSpec(DamageEffectClass, 1.0f, SourceASC->MakeEffectContext());
		...
		for (int32 i = 0; i < NumHits; ++i)
		{
			...
			if (HasAuthority() && TargetASC && UBaseAbilitySystemLibrary::IsHostile(OwnerPawn, FireHit.GetActor()))
			{
				...
				DamageEffectParams.ShowDamageDelay = FMath::FRandRange(0.f, MaxDelay);
